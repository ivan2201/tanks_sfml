...
void Game::start(sf::RenderWindow & window)
{
  ...
  if ((m_windowRect.top - m_windowRect.bottom) > (m_windowRect.right - m_windowRect.left))
  {
    m_field.setBoundSquare(
    {
      static_cast < us >(m_windowRect.left),
      static_cast < us >(m_windowRect.top + 
          (m_windowRect.bottom + m_windowRect.left - m_windowRect.top - m_windowRect.right) / 2),
      static_cast < us >(m_windowRect.right - m_windowRect.left)
    });
  }
  else
  {
    ...
    {
      static_cast< us >(m_windowRect.left +
          (m_windowRect.right - m_windowRect.left + m_windowRect.top - m_windowRect.bottom) / 2),
      static_cast< us >(m_windowRect.top),
      static_cast< us >(m_windowRect.bottom - m_windowRect.top)
    });
  }
  ...
  {
    if (x > (Game::getGame().getWindowRect().right / 2 - menuItem.getLocalBounds().width / 2)
        && x < (Game::getGame().getWindowRect().right / 2 + menuItem.getLocalBounds().width / 2))
    {
      if (y > (Game::getGame().getWindowRect().bottom / 3 - menuItem.getLocalBounds().height / 2)
          && y < (Game::getGame().getWindowRect().bottom / 3 + menuItem.getLocalBounds().height / 2))
      {
        isContinued = true;
      } 
      else if (y > (Game::getGame().getWindowRect().bottom * 2 / 3 - menuItem.getLocalBounds().height / 2)
          && y < (Game::getGame().getWindowRect().bottom * 2 / 3 + menuItem.getLocalBounds().height / 2))
      {
        exitBool = true;
      }
    }
  ...
  while (message.message != WM_QUIT && !exitBool && !gameFinished)
  {
    {
      ...
      while (message.message != WM_QUIT && !escPressed)
      {
        ...
        {
          frameDelay(1.0f / FRAMERATE_LIMIT);
    ...
    redrawField(window);
    while (message.message != WM_QUIT && !escPressed && !gameFinished)
    {
      if (PeekMessage(&message, NULL, 0, 0, PM_REMOVE))
      {
        // If a message was waiting in the message queue, process it
      ...
      else
      {
        ...
        // Clear views
        window.clear();
        point_t prevPoint = {static_cast< us >(m_myTank.getX()), 
        ...
        if (prevPoint.x != static_cast< us >(m_myTank.getX()) || 
            prevPoint.y != static_cast< us >(m_myTank.getY())) updatePath();
        ...
        while (iterEnemies != end)
        {
          if (iterEnemies->mayDelete())
          {
            ...
            if (enemiesLeft > 4)
            {
              lastGeneredEnemyPosition = (lastGeneredEnemyPosition + 1) % 5;
              m_preEnemy.setPosition({static_cast< us >(4 * (lastGeneredEnemyPosition + 1)), 1});
          ...
          else
          {
            ...
            if (rand() < 100 && !iterEnemies->isDestroyed())
            {
        ...
        while (iterBull != m_bullets.end())
        {
          ...
          const int left = static_cast< int >(iterBull->getX() - width);
          const int right = static_cast< int >(iterBull->getX() + width);
          const int up = static_cast< int >(iterBull->getY() - height);
          const int down = static_cast< int >(iterBull->getY() + height);
          ...
          if ((m_field[down][right].type & BIT_UNSHOOTED) != 0)
          {
            if ((m_field[down][right].type) == TYPE_BRICK)
            {
              destroyedCell = &m_field[down][right];
            }
            bulletDestr = true;
          }
          else if ((m_field[down][left].type & BIT_UNSHOOTED) != 0)
          {
            if ((m_field[down][left].type) == TYPE_BRICK)
            {
              destroyedCell = &m_field[down][left];
            }
            bulletDestr = true;
          }
          else if ((m_field[up][right].type & BIT_UNSHOOTED) != 0)
          {
            if ((m_field[up][right].type) == TYPE_BRICK)
            {
              destroyedCell = &m_field[up][right];
            }
            bulletDestr = true;
          }
          else if ((m_field[up][left].type & BIT_UNSHOOTED) != 0)
          {
            if ((m_field[up][left].type) == TYPE_BRICK)
            {
              destroyedCell = &m_field[up][left];
            }
            bulletDestr = true;
          }
          if (bulletDestr)
    ...
    if (gameFinished)
    {
      sf::Text finalText("you won", font, (m_windowRect.bottom - m_windowRect.top) / 8);
      finalText.setPosition(m_windowRect.left + (m_windowRect.right - m_windowRect.left) / 2,
          m_windowRect.top + (m_windowRect.bottom - m_windowRect.top) / 2);
      finalText.setOrigin(sf::Vector2f(finalText.getGlobalBounds().width / 2, finalText.getGlobalBounds().height / 2));
      if (m_myTank.isDestroyed())
      {
        finalText.setString("you lose");
        finalText.setColor(sf::Color(0xFF,0x00,0x00,0xFF));
        while (message.message != WM_QUIT && !m_myTank.mayDelete())
        {
          frameDelay(1.0f / FRAMERATE_LIMIT);
          float time = clock.getElapsedTime().asSeconds();
          float delta = time - previousTime;
          previousTime = time;
          m_myTank.move(0, delta);
          redrawField(window);
    ...
    while (message.message != WM_QUIT && !escPressed && !exitBool && !isContinued && gameFinished && !spacePressed)
    {
      if (PeekMessage(&message, NULL, 0, 0, PM_REMOVE))
      {
        // If a message was waiting in the message queue, process it
    ...
    escPressed = false;
    eventController.setLMouseClickListener(menuOnClickListener);
    ...
    window.clear();
    menuItem.setPosition(m_windowRect.right / 2, m_windowRect.bottom / 3);
    menuItem.setOrigin(sf::Vector2f(menuItem.getTexture()->getSize()) / 2.f);
    continue_game.setPosition(m_windowRect.right / 2 - continue_game.getGlobalBounds().width / 2, 
                              m_windowRect.bottom / 3 - continue_game.getGlobalBounds().height);
    ...
    menuItem.setPosition(m_windowRect.right / 2, m_windowRect.bottom * 2 / 3);
    exit.setPosition(m_windowRect.right / 2 - exit.getGlobalBounds().width / 2, 
                     m_windowRect.bottom * 2 / 3 - exit.getGlobalBounds().height);
    window.draw(menuItem);
    window.draw(continue_game);
    window.draw(exit);
    window.display();
    while (message.message != WM_QUIT && !escPressed && !exitBool && !isContinued && !gameFinished)
    {
      if (PeekMessage(&message, NULL, 0, 0, PM_REMOVE))
      {
        // If a message was waiting in the message queue, process it
  ...
  eventController.setLMouseClickListener(nullptr);
}
...
void Game::generateField(const ull &seed)
{
  ...
  for (int i = 0; i < 25; i++)
  {
    for (int j = 0; j < 25; j++)
    {
      ...
      sqr = (1027 + rand2 * sqr) % 25037;
      if (sqr < 3000)
      {
        m_field[i][j].type = TYPE_BETON;
      } 
      else if (sqr < 11000)
      {
        m_field[i][j].type = TYPE_BRICK;
      }
      else if (sqr < 14000)
      {
        m_field[i][j].type = TYPE_HADGEHOG;
      }
      else
      {
        m_field[i][j].type = TYPE_GROUND;
      }
      randCount++;
    }
  }
  for (int i = 0; i < 24; i++)
  {
    ...
    m_field[i][24].type = TYPE_BETON;
    m_field[24][24 - i].type = TYPE_BETON;
    m_field[24 - i][0].type = TYPE_BETON;
  }
  m_field[23][12].type = TYPE_SPAWN;
  for (int i = 4; i < 21; i= i + 4)
  {
    m_field[1][i].type = TYPE_SPAWN;
  }
  for (int i = 5; i < 21; i++)
  {
    if (m_field[1][i].type == TYPE_BETON)
    {
      m_field[1][i].type = TYPE_BRICK;
    }
    else if (m_field[1][i].type == TYPE_HADGEHOG)
    {
      m_field[1][i].type = TYPE_GROUND;
    }
  }
  updatePath(m_path, {12, 23});
  bool isPath = true;
  if (m_path[1][4] == Field::NOPATH)
  {
    isPath = false;
  }
  if (!isPath)
  {
    ...
    updatePath(pathFromEnemySpawn, {12, 1});
    point_t maxDistancePoint = {12,23}; 
    us maxDistance = m_path[23][12];
    for (us i = 0; i < FIELD_WIDTH; i++)
    {
      for (us j = 0; j < FIELD_WIDTH; j++)
      {
        if (m_path[i][j] != Field::NOPATH && maxDistance < m_path[i][j])
        {
          maxDistance = m_path[i][j];
          maxDistancePoint = {j, i}; 
        }
      }
    }
    ...
    {
      if (m_field[maxDistancePoint.y][i].type == TYPE_BETON)
      {
        m_field[maxDistancePoint.y][i].type = TYPE_BRICK;
      }
      else if (m_field[maxDistancePoint.y][i].type == TYPE_HADGEHOG)
      {
        m_field[maxDistancePoint.y][i].type = TYPE_GROUND;
      }
    }
    for (int i = maxDistancePoint.y + 1; i <= end.y; i++)
    {
      if (m_field[i][end.x].type == TYPE_BETON)
      {
        m_field[i][end.x].type = TYPE_BRICK;
      }
      else if (m_field[i][end.x].type == TYPE_HADGEHOG)
      {
        m_field[i][end.x].type = TYPE_GROUND;
      }
    }
    updatePath(m_path, { 12, 23 });
  }
  for (us i = 0; i < FIELD_WIDTH; i++)
  {
    for (us j = 0; j < FIELD_WIDTH; j++)
    {
      switch (m_field[i][j].type)
      {
      case TYPE_BETON:
        m_field[i][j].image.setTexture(m_textures[0]);
        break;
      case TYPE_GROUND:
        m_field[i][j].image.setTexture(m_textures[2]);
        break;
      case TYPE_HADGEHOG:
        m_field[i][j].image.setTexture(m_textures[3]);
        break;
      case TYPE_BRICK:
        m_field[i][j].image.setTexture(m_textures[1]);
        m_field[i][j].health = 3;
        break;
      default:
        if (m_path[i][j] == 0)
          m_field[i][j].image.setTexture(m_textures[8]);
        else
          m_field[i][j].image.setTexture(m_textures[7]);
      }
      m_field[i][j].image.setColor(sf::Color(0xFF,0xFF,0xFF,0xFF));
      m_field[i][j].image.setPosition(
          m_field.getBoundSquare().left + j * m_field.getCellLength(),
          m_field.getBoundSquare().top + i * m_field.getCellLength());
      auto bound = m_field[i][j].image.getLocalBounds();
      m_field[i][j].image.setScale(m_field.getCellLength() / bound.width, 
          m_field.getCellLength() / bound.height);
    }
  }
  m_myTank.setPosition({ 12, 23 });
  for (us i = 4; i < 21; i += 4)
  {
...
void Game::redrawField(sf::RenderWindow & window)
{
  for (int i = 0; i < FIELD_WIDTH; i++)
  {
    ...
    {
      if (m_field[i][j].type == TYPE_BRICK)
      {
        m_field[i][j].image.setColor(
            sf::Color(0xFF,0xFF,0xFF, static_cast<int>(0xFF - 0x7F * (3 - m_field[i][j].health) / 3)));
      }
      window.draw(m_field[i][j].image);
    }
  }
...
void Game::updatePath(us (&path)[FIELD_WIDTH][FIELD_WIDTH], point_t begin)
{
  for (int i = 0; i < FIELD_WIDTH; i++)
    for (int j = 0; j < FIELD_WIDTH; j++)
      path[i][j] = Field::NOPATH;
  std::queue< point_t > queue;
  path[begin.y][begin.x] = 0;
  queue.push(begin);
  ...
  while (!queue.empty())
  {
    ...
    queue.pop();
    if ((curPoint.y > 0) && (path[curPoint.y - 1][curPoint.x] == Field::NOPATH)
        && !((m_field[curPoint.y - 1][curPoint.x].type & TYPE_UNPATHCELL) == TYPE_UNPATHCELL))
    {
      path[curPoint.y - 1][curPoint.x] = path[curPoint.y][curPoint.x] + 1;
      queue.push({curPoint.x, static_cast< us >(curPoint.y - 1)});
    }
    if ((curPoint.x > 0) && (path[curPoint.y][curPoint.x - 1] == Field::NOPATH)
        && !((m_field[curPoint.y][curPoint.x - 1].type & TYPE_UNPATHCELL) == TYPE_UNPATHCELL))
    {
      path[curPoint.y][curPoint.x - 1] = path[curPoint.y][curPoint.x] + 1;
      queue.push({static_cast< us >(curPoint.x - 1), curPoint.y});
    }
    if ((curPoint.y + 1 < FIELD_WIDTH) && (path[curPoint.y + 1][curPoint.x] == Field::NOPATH)
        && !((m_field[curPoint.y + 1][curPoint.x].type & TYPE_UNPATHCELL) == TYPE_UNPATHCELL))
    {
      path[curPoint.y + 1][curPoint.x] = path[curPoint.y][curPoint.x] + 1;
      queue.push({curPoint.x, static_cast< us >(curPoint.y + 1)});
    }
    if ((curPoint.x + 1 < FIELD_WIDTH) && (path[curPoint.y][curPoint.x + 1] == Field::NOPATH)
        && !((m_field[curPoint.y][curPoint.x + 1].type & TYPE_UNPATHCELL) == TYPE_UNPATHCELL))
    {
      path[curPoint.y][curPoint.x + 1] = path[curPoint.y][curPoint.x] + 1;
      queue.push({static_cast< us >(curPoint.x + 1), curPoint.y});
...
point_t findNearPathPoint(us (&path)[FIELD_WIDTH][FIELD_WIDTH], point_t begin)
{
  for (int curDistance = 1; curDistance < 2 * FIELD_WIDTH; curDistance++)
  {
    for (int dx = -curDistance; dx <= curDistance; dx++)
    {
      ...
      if (y1 < FIELD_WIDTH && path[y1][x] != Field::NOPATH)
      {
        return {static_cast< us >(x), static_cast< us >(y1)};
      }
      if (y2 > -1 && path[y2][x] != Field::NOPATH)
      {
...
void frameDelay(float delay)
{
  ...
  static float prevTime = clock.getElapsedTime().asSeconds();
  while ((clock.getElapsedTime().asSeconds() - prevTime) < delay);
